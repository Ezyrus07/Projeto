(() => {
  ...
  function getProfileGlyphMarkup(extraClass = "") {
    ...
    return `
      <span class="profile-glyph ${cls}" aria-hidden="true">
  ...
  function buildHeaderProgressState(state) {
    ...
    const rawPoints = Number(
      ...
      profile.xp_total ??
      profile.score ??
      (state?.authenticated ? 12450 : 320)
    ) || 0;
    const rawLevel = Math.max(1, Number(
      profile.nivel ??
      profile.level ??
      profile.lv ??
      (state?.authenticated ? Math.floor(rawPoints / 520) + 1 : 1)
    ) || 1);
  ...
  function buildHeaderStatusMarkup(state) {
    if (!state?.authenticated) return "";
    const progress = buildHeaderProgressState(state);
    const label = "Pontos";
    const subtitle = `Nivel ${progress.levelLabel}`;
    const helper = progress.xpLabel;
    return `
      <div class="profile-status-card" data-v2-role="profile-status">
        <div class="profile-status-points">
          <span class="eyebrow">${label}</span>
          <strong>${progress.pointsLabel}</strong>
        </div>
        <div class="profile-status-progress">
          <div class="profile-status-head">
            <span class="level">${subtitle}</span>
            <span class="xp">${helper}</span>
          </div>
  ...
  function buildAuthState(profile, authenticated, source) {
    ...
    const avatar = authenticated ? readAvatar(normalizedProfile) : "";
    const state = {
  ...
  function createShell() {
      ...
      <header class="doke-v2-header">
        ...
        <div class="header-right">
          <div class="mobile-actions">
            ...
            <a href="pedidos.html" data-v2-link class="icon-btn" aria-label="Pedidos"><i class='bx bx-package'></i></a>
            <a href="mensagens.html" data-v2-link class="icon-btn" aria-label="Mensagens"><i class='bx bx-message-rounded-dots'></i></a>
          </div>

          <div class="profile-shell ${authState.authenticated ? "is-authenticated" : "is-guest"}">
            ${buildHeaderStatusMarkup(authState)}
          <div class="profile">
            <button type="button" class="profile-btn" aria-label="Perfil" data-v2-role="profile-button" data-profile-href="${authState.profileHref}" data-authenticated="${authState.authenticated ? "1" : "0"}">
              ${authState.avatar ? `<img src="${authState.avatar}" alt="Perfil" onerror="this.onerror=null;this.src='assets/Imagens/user_placeholder.png'">` : getProfileGlyphMarkup("is-header")}
            </button>
    ...
    function renderAuthUi(nextState) {
      ...
      if (profileBtn instanceof HTMLButtonElement && !authState.avatar) {
        profileBtn.innerHTML = getProfileGlyphMarkup("is-header");
      } else if (profileBtn instanceof HTMLButtonElement && authState.avatar && !(profileBtn.querySelector("img") instanceof HTMLImageElement)) {
        profileBtn.innerHTML = `<img src="${authState.avatar}" alt="Perfil" onerror="this.onerror=null;this.src='assets/Imagens/user_placeholder.png'">`;
      }
      const profileStatus = root.querySelector("[data-v2-role='profile-status']");
      if (profileStatus instanceof HTMLElement) {
        profileStatus.outerHTML = buildHeaderStatusMarkup(authState);
      }
    ...
    const shellApi = { root, main, setActive };
    try { window.__DOKE_V2_ACTIVE_SHELL__ = shellApi; } catch (_e) {}
    return shellApi;
  }
