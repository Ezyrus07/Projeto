...
(() => {
  ...
  async function updateFriendButton(ctx){
    ...
    const setupMessageButton = () => {
      ...
      msgBtn.onclick = () => {
        if (!canSend) {
          redirectPerfilLogin("meuperfil.html");
          return;
        }
...
(function(){
  ...
  function redirectPerfilLogin(nextHref){
    try {
      const next = String(nextHref || ((location.pathname || '').split('/').pop() || 'meuperfil.html'));
      const href = `login.html?noshell=1&next=${encodeURIComponent(next)}`;
      try { window.top.location.replace(href); } catch(_){ window.location.replace(href); }
    } catch(_) { try { window.location.replace('login.html?noshell=1&next=meuperfil.html'); } catch(__){} }
  }
  "use strict";
