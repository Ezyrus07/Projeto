...
(function(){
  ...
  window.deleteUser = async function(_userIgnored){
    const sb = ensure();
    // Supabase não permite delete de usuario via client sem server-side.
    const { error } = await sb.auth.signOut();
    if (error) throw error;
    setCurrentUser(null);
    console.warn("[DOKE] deleteUser: operacao no client não suportada; usuario apenas deslogado.");
    return true;
  };
